    ...
    <div style={{ minHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: "var(--pearl)" }}>
      <div className="fade-up" style={{ width: "100%", maxWidth: 380 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <Logo size={64} />
          <div className="font-display" style={{ fontSize: 32, fontWeight: 600, color: "var(--charcoal)", marginTop: 14 }}>Admin Access</div>
          <div style={{ fontSize: 13, color: "var(--text-muted)", fontFamily: "'Jost',sans-serif", marginTop: 6 }}>Enter password to manage products</div>
        </div>
        <div style={{ background: "#fff", padding: 32, borderRadius: 22, boxShadow: "0 8px 40px rgba(0,0,0,.1)" }}>
          <label style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--charcoal)", fontFamily: "'Jost',sans-serif", display: "block", marginBottom: 8 }}>Password</label>
          <input type="password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && tryLogin()}
            placeholder="Enter admin password"
            style={{ width: "100%", padding: "12px 16px", borderRadius: 12, border: `1.5px solid ${pwErr ? "#e53e3e" : "rgba(201,168,76,.3)"}`, fontFamily: "'Jost',sans-serif", fontSize: 14, marginBottom: 8 }} />
          {pwErr && <div style={{ color: "#e53e3e", fontSize: 12, fontFamily: "'Jost',sans-serif", marginBottom: 10 }}>❌ Incorrect password</div>}
          <button className="btn-gold" onClick={tryLogin} style={{ width: "100%", padding: 13, borderRadius: 14, fontFamily: "'Jost',sans-serif", fontSize: 14 }}>Login to Admin</button>
          <button onClick={onBack} style={{ display: "block", width: "100%", marginTop: 12, padding: 10, background: "none", border: "none", cursor: "pointer", fontFamily: "'Jost',sans-serif", fontSize: 13, color: "var(--text-muted)" }}>← Back to Store</button>
        </div>
        <div style={{ textAlign: "center", marginTop: 14, fontSize: 12, color: "var(--text-muted)", fontFamily: "'Jost',sans-serif" }}>Demo password: <strong>faraz2025</strong></div>
      </div>
    </div>
  );
              ...
              <tbody>
                {products.map((p, i) => (
                  <tr key={p.id} className="admin-row" style={{ borderBottom: "1px solid rgba(201,168,76,.08)", background: i % 2 ? "rgba(250,248,245,.5)" : "#fff" }}>
                    ...
                    <td style={{ padding: "14px 18px", fontFamily: "'Jost',sans-serif", fontSize: 13 }}>{p.category}</td>
                    <td style={{ padding: "14px 18px" }}><span className="font-display" style={{ fontSize: 18, fontWeight: 600 }}>₹{p.price.toLocaleString()}</span></td>
                    <td style={{ padding: "14px 18px", fontFamily: "'Jost',sans-serif", fontSize: 13, color: "var(--text-muted)" }}>{p.fabric}</td>
                    <td style={{ padding: "14px 18px" }}><div style={{ display: "flex", alignItems: "center", gap: 5 }}><Stars r={p.rating} /><span style={{ fontSize: 12, fontFamily: "'Jost',sans-serif" }}>{p.rating}</span></div></td>
                    <td style={{ padding: "14px 18px" }}>
